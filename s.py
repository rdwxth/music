...
def get_json_structure(data, indent=2):
  """
  Get the structure of a JSON file and save it as a text file.

  Parameters:
  - data: JSON data to analyze.
  - indent: Indentation level for the text file (default is 2).
  """
  def _get_structure(obj, current_indent=0):
      structure = ""
      if isinstance(obj, dict):
          structure += "{\n"
          for key, value in obj.items():
              structure += " " * (current_indent + indent) + f'"{key}": {_get_structure(value, current_indent + indent)}'
              structure += ",\n"
          structure += " " * current_indent + "}"
      elif isinstance(obj, list):
          if obj:  # Check if the list is not empty
              structure += "[\n"
              structure += " " * (current_indent + indent) + f"{_get_structure(obj[0], current_indent + indent)}"
              structure += "\n" + " " * current_indent + "]"
          else:
              structure += "[]"
      else:
          structure += '"<content>"'  # Placeholder for non-dict and non-list content
      return structure

  json_structure = _get_structure(data)

  # Save the structure as a text file
  with open("json_structure.txt", "w") as file:
      file.write(json_structure)

# Example usage:
# Replace 'your_json_data' with the actual JSON data you want to analyze
your_json_data = {
  "status": "SUCCESS",
  "message": "null",
  "data": {
...
get_json_structure(your_json_data)
